...
`tools/crosscheck_coverage.py` compares this list against the ATT&CK
technique tags actually present in `packs/windows/{essential,advanced,hunting}`
...
weighted by rank/score so the highest-priority gaps surface first. Entries
with `scoring_status: pending_rescore` are excluded from the ranked stats and
