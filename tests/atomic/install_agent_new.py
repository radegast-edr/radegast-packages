...
def main():
    parser = argparse.ArgumentParser(description="Radegast EDR Agent & Rustinel Windows Installer")
    parser.add_argument("--name", default=platform.node(), help="Device name to register (default: hostname)")
    parser.add_argument("--group-id", type=int, default=10, help="Device group ID (default: 10)")
    parser.add_argument("--uninstall", action="store_true", help="Unregister device and run uninstall.bat")
    ...
    if args.uninstall:
        print("=== Starting Radegast EDR Agent & Rustinel Windows Uninstallation ===")
        api_key = os.environ.get("RADEGAST_KEY")
        if not api_key:
            print("ERROR: RADEGAST_KEY environment variable is not set.", file=sys.stderr)
            sys.exit(1)

        device_json = radegast_dir / "device.json"
        if not device_json.exists():
            print(f"ERROR: {device_json} not found. Cannot unregister device.", file=sys.stderr)
            sys.exit(1)
        device_id = json.loads(device_json.read_text(encoding="utf-8")).get("device_id")
        if not device_id:
            print("ERROR: device_id not found in device.json.", file=sys.stderr)
            sys.exit(1)

        print(f"Unregistering device (ID: {device_id})...")
        try:
            conn = http.client.HTTPSConnection("console-api.radegast.app", context=ssl_context)
            conn.request("DELETE", f"/api/v1/devices/{device_id}", headers={
            ...
            if response.status not in (200, 204):
                print(f"WARNING: API returned {response.status} when unregistering device.", file=sys.stderr)
            else:
                print("Device unregistered successfully.")
        except Exception as e:
            print(f"WARNING: Failed to unregister device: {e}", file=sys.stderr)

        uninstall_bat = radegast_dir / "uninstall.bat"
        if not uninstall_bat.exists():
            print(f"ERROR: uninstall.bat not found at {uninstall_bat}", file=sys.stderr)
            sys.exit(1)
        print("Running uninstall.bat...")
        subprocess.run([str(uninstall_bat)], input=b"y\n", check=True)
        sys.exit(0)

    # 0. Register device and obtain token via API
    api_key = os.environ.get("RADEGAST_KEY")
    if not api_key:
        print("ERROR: RADEGAST_KEY environment variable is not set.", file=sys.stderr)
        sys.exit(1)

    print(f"Registering device '{args.name}' (group {args.group_id}) with the Radegast console...")
    try:
        payload = json.dumps({"name": args.name, "group_id": args.group_id})
        conn = http.client.HTTPSConnection("console-api.radegast.app", context=ssl_context)
        conn.request("POST", "/api/v1/devices/", body=payload, headers={
            "accept": "application/json",
        ...
        if response.status != 200 and response.status != 201:
            print(f"ERROR: API returned {response.status}: {response_data}", file=sys.stderr)
            sys.exit(1)
        token = response_data.get("token")
        device_id = response_data.get("id")
        if not token:
            print("ERROR: No token received in API response.", file=sys.stderr)
            sys.exit(1)
        print(f"Device registered successfully (ID: {device_id}).")
    except Exception as e:
        print(f"ERROR: Failed to register device: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    (radegast_dir / "device.json").write_text(json.dumps({"device_id": device_id}), encoding="utf-8")
    ...
    agent_xml = f"""<service>
      ...
      <env name="RADEGAST_AGENT_DEVICE_ID" value="{device_id}" />
